...
const float ALIGNMENT_DISTANCE = 00.0f;
const float COHESION_DISTANCE = 000.0f;
...
void update_agent(Agent *agent, Agent *agents, int agent_count) {
    // Separation, alignment, and cohesion vectors
    float sx = 0, sy = 0;
    float ax = 0, ay = 0;
    float cx = 0, cy = 0;
    ...
    // Update agent velocity based on separation, alignment, and cohesion
    agent->vx += sx + ax + cx;
    agent->vy += sy + ay + cy;
...
int main(int argc, char *argv[]) {
    ...
    while (running) {
        ...
        for (int i = 0; i < AGENT_COUNT; i++) {
            update_agent(&agents[i], agents, AGENT_COUNT);
            move_agent(&agents[i]);
